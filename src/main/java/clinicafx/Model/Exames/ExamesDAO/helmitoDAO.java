...
import clinicafx.Model.Exames.model_helmito;
...
public class helmitoDAO {
    ...
    public helmitoDAO(MainApp mainapp) {
        if (mainapp != null) {
            try {
    ...
    public model_helmito getHelmitoID(int ID, MainApp mainapp) {
        db.open();
        model_helmito obj = new model_helmito(mainapp);
...
//                obj.setFoto(rs.getString("foto"));//10
            ...
            db.close();
            MsgErro.MessagemErroBD(ex, "getHelmitoID");
            return null;
        } catch (Exception ex) {
            db.close();
            MsgErro.MessagemErroBD(ex, "getHelmitoID");
    ...
    public List<model_helmito> getHelmitoList(MainApp mainapp) {
        db.open();
        ArrayList<model_helmito> result = new ArrayList<>();
        try {
...
//                obj.setFoto(rs.getString("foto"));//10
            ...
            db.close();
            MsgErro.MessagemErroBD(ex, "getHelmitoList");
            return result;
    ...
    public void creatHelmito(model_helmito obj) {
        db.open();
        try {
            stmt = conexao.prepareStatement("INSERT INTO clientes ( `ID_fezes`, `a_lumbricoides`, `t_trichiura`, `e_vermicularis`, `ancilostomideos`, `s_stercoralis`, `taenia_sp`, `outros`) VALUES(?,?,?,?,?,?,?,?,?,?);");
...
//            stmt.setString(10, obj.getFoto());
            ...
            db.close();
            MsgErro.MessagemErroBD(ex, "creatHelmito");
        } catch (Exception ex) {
            db.close();
            MsgErro.MessagemErroBD(ex, "creatHelmito");
    ...
    public void updateHelmito(model_helmito obj) {
        db.open();
        try {
...
//            stmt.setInt(10, obj.getID_cliente());
            ...
            db.close();
            MsgErro.MessagemErroBD(ex, "updateHelmito");
        } catch (Exception ex) {
            db.close();
            MsgErro.MessagemErroBD(ex, "updateHelmito");
    ...
    public void deleteHelmito(model_helmito obj) {
        db.open();
        try {
            stmt = conexao.prepareStatement("DELETE FROM clientes WHERE ID_cliente = ?;");

            stmt.setInt(1, obj.getID_helmito());

            stmt.executeUpdate();

        } catch (SQLException ex) {
            db.close();
            MsgErro.MessagemErroBD(ex, "deleteHelmito");
        } catch (Exception ex) {
            db.close();
            MsgErro.MessagemErroBD(ex, "deleteHelmito");
